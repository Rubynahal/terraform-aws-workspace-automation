...
#####################
resource "aws_iam_policy" "workspace_creation" {
  count = local.create_role && var.attach_workspace_creation_policy ? 1 : 0

  name   = "${local.role_name}-workspace_creation"
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [
      {
...
resource "aws_iam_policy_attachment" "workspace_creation" {
  count = local.create_role && var.attach_workspace_creation_policy ? 1 : 0

  name       = "${local.role_name}-workspace_creation"
  roles      = [aws_iam_role.lambda[0].name]
  policy_arn = aws_iam_policy.workspace_creation[0].arn
}
...
data "aws_iam_policy_document" "kms_key" {
  count = local.create_role && var.attach_kms_key_policy ? 1 : 0
  ...
  statement {
    ...
    resources = var.policy_kms_key_arns
  }
}
...
resource "aws_iam_policy_attachment" "kms_key" {
  count = local.create_role && var.attach_kms_key_policy ? 1 : 0

  name       = "${local.role_name}-kms"
  roles      = [aws_iam_role.lambda[0].name]
  policy_arn = aws_iam_policy.kms_key[0].arn
}
